...
directory = "/Users/ahmedmoamen/Desktop/ahmed/work/Programming/python/NPL/USA presidential vocab/presidents"
...
def get_president_sentences(president):
    files = sorted([file for file in os.listdir(directory) if president.lower() in file.lower()])
    speeches = [read_file(directory+ "/"+ file) for file in files]
...
def most_frequent_words(list_of_sentences):
    ...
    most = Counter(all_words).most_common()
    result = list()
    for (word, number) in most:
        result.append(word)
    return result
